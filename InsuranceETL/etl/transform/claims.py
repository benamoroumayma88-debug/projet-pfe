...
def transform_claims(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    # FIX: Fill NaN values in montant_indemnisation_claim (required non-NULL column)
    if "montant_indemnisation_claim" in df.columns:
        # If montant_indemnisation is NULL but montant_estime exists, use estimate
        if "montant_estime_dommage_claim" in df.columns:
            mask_null = df["montant_indemnisation_claim"].isna()
            df.loc[mask_null, "montant_indemnisation_claim"] = df.loc[mask_null, "montant_estime_dommage_claim"]
        
        # For remaining NaN values (status = Refusé or no estimate), use 0
        df["montant_indemnisation_claim"] = df["montant_indemnisation_claim"].fillna(0)
